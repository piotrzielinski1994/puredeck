...
// AC-009 parity: puredeck renders the hoisted pureui UpdatesSection wired with
// its REAL toast seam (the untouched createPuredeckUpdateToastSink over the
// ToastProvider's `show`, and both one-shot messages routed through `show`).
// puredeck renders toasts to the DOM, so the observable contract is asserted on
// rendered text, proving its styling is preserved end-to-end.
...
function renderSection(
  ...
  return render(
    <ToastProvider>
      <Harness controller={controller} getVersion={getVersion} />
    </ToastProvider>,
  );
}

describe("UpdatesSection (puredeck consume)", () => {
  // behavior: renders the current version string from the injected source
  ...
  // behavior: check reports no update -> "latest" toast via `show` + button idle
  ...
  // behavior: update found -> update toast (version + Update now) via the sink
  ...
  // behavior: check rejects -> "check failed" toast via `show` + button not stuck
  ...
  // side-effect-contract: in-flight guard - the button is disabled while a check
  // is pending and a second click does not start a second check
