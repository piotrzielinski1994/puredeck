...
// platform directly and use metaKey. The recorder HOOK calls detectPlatform()
// internally (jsdom reports non-mac -> Mod resolves to Control, per learnings),
// so the hook tests dispatch Control-based events to land on the canonical
// "Mod+.." string.

describe("eventToHotkey (AC-003 / TC-006)", () => {
  // AC-003, TC-006 — behavior: the composed macOS Option glyph must NOT be
  // recorded; the physical key from event.code is what the matcher fires on.
  ...
  // AC-003 — behavior: a remapped layout (Dvorak) fires key="l" while code=KeyP;
  ...
  // AC-003, TC-007 — behavior: modifier-only presses are ignored.
  ...
  it("should return null if the event is a Control modifier-only press", () => {
    expect(
      eventToHotkey({ ctrlKey: true, key: "Control", code: "ControlLeft" }, "mac"),
    ).toBeNull();
  });
  ...
  // AC-003 — behavior: Option-composed punctuation uses the PUNCTUATION_CODE_MAP
  // fallback (⌘⌥- fires key="–" but code="Minus").
  ...
  // AC-003, TC-006 — side-effect-contract: the recorder's output must be exactly
  // what the matcher fires on. Round-trip the SAME real event through both.
  ...
  // AC-003 — side-effect-contract: same round-trip for the punctuation fallback.
...
describe("useRecordHotkey (AC-003 / TC-006 / TC-007 / TC-008)", () => {
  // AC-003 — behavior
  ...
  // AC-003, TC-006 — side-effect-contract: a composed combo keydown records the
  ...
  // AC-003, TC-008 — side-effect-contract: Escape aborts (onCancel), records nothing.
  ...
  // AC-003, TC-007 — behavior: a modifier-only keydown is ignored and recording
  // continues (the recorder keeps listening).
