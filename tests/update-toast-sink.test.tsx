...
// AC-009 parity: puredeck wires the hoisted pureui updater flow to its OWN
// ToastProvider via createPuredeckUpdateToastSink. This test drives the real
...
// ToastProvider's `show` - the same wiring RootLayout uses.
function CheckerHarness({ controller }: { controller: UpdateController }) {
  const { show } = useToast();
  const sink = createPuredeckUpdateToastSink(show);
...
describe("puredeck update toast sink parity", () => {
  // behavior: an available update shows a persistent toast with the version text
  ...
  // side-effect-contract: clicking Update now invokes downloadAndInstall,
  ...
  // behavior: once download starts, the Update now button is replaced by the
  // progress label so it can't be re-fired mid-download.
  ...
  // behavior: the available toast is persistent - it survives past the 2500ms
  // auto-dismiss window.
  ...
  // behavior/side-effect-contract: clicking × removes the toast and does NOT
  // invoke the install path.
