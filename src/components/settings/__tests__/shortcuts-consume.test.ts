...
// R6b consume guard (TC-015): puredeck deleted its local shortcut-row.tsx +
// shortcuts-section.tsx and resolves ShortcutsSection from @pziel/pureui (with
// className="p-6"), while keeping its own registry.ts + resolve.ts untouched.
// Reads the source tree off disk, never trusting a mock.
...
describe("puredeck consumes the pureui shortcuts UI (TC-015)", () => {
  ...
  it("should preserve its p-6 section padding", () => {
    const source = readFileSync(settingsViewPath, "utf8");

    expect(source).toMatch(/className="p-6"/);
  });
